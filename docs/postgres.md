...
PostgreSQL 18 is the first release to issue asynchronous I/O, and with
`io_method = io_uring` it submits reads through io_uring rather than
blocking in `pread`. That moves a real part of query latency below the line
`EXPLAIN (ANALYZE, BUFFERS)` can see: `EXPLAIN` tells you a scan read
324,991 blocks, not that 94% of those reads detoured through kernel worker
threads on the way back.

This guide shows what uringscope sees on a stock PostgreSQL 18, which knobs
move it, and — importantly — the several ordinary situations where the
report is legitimately empty.
...
# the load-bearing check: this build must actually have io_uring support
ldd /usr/lib/postgresql/18/bin/postgres | grep uring
#   liburing.so.2 => /lib/x86_64-linux-gnu/liburing.so.2
```

`io_method` **defaults to `worker`, not `io_uring`** — asynchronous, but via
a pool of helper processes rather than the kernel. Switch it explicitly; it
is postmaster-context, so the cluster must restart:
...
syscalls outright, so a PostgreSQL running under stock Docker is not using
io_uring at all regardless of `io_method`. Bare metal and VMs are the
interesting targets.

## What PostgreSQL 18 actually sends through io_uring

This is the first thing to internalise, because it determines whether
uringscope has anything to tell you. Measured, not inferred:
...
PostgreSQL 18's AIO is wired into its **read-stream** paths — the ones that
know what they will need next and can queue it. A 46,680-transaction
`pgbench` run doing random single-row `UPDATE`s produced **zero** io_uring
...
`io_method = io_uring` set correctly.** That is not a failed attach. It
earns its keep on analytical and bulk-read work.
...
- uringscope's report lists rings as `comm=postgres` without a pid, so with
  several busy backends you cannot currently attribute submissions to a
  particular session. Use `-p` if you need per-backend numbers.
...
Always check the plan before concluding the tool is broken.
...
If that is green and PostgreSQL's report is empty, the empty report is real.
...
**Read the latency, not the punt rate.** The intuition that a warm cache
eliminates punting is wrong: it falls only 20 points, and the worker count
does not move at all. What changes is what a punt *costs* — 18x on average.
A high punt rate on buffered reads is normal; a high punt rate with a cold
cache is where your p99 lives.

`untracked_completions` was 0 in both runs, so these are complete
reconstructions rather than sampled estimates.
...
identical bytes. Note the trap: punt rate *falls* and per-request latency
*improves*, because each request is now a sixteenth the size. uringscope's
`--diff` will report this as `IOPS +578%`, `punt rate fell`, `READ p50
-87.5%` — every line reading like a win, for what is plainly a
pessimization. **Compare submissions and syscalls alongside latency**; a
rate that improves because the unit of work shrank is not an improvement.
...
At 1, PostgreSQL stops using io_uring altogether and falls back to
...
io_uring is the fastest of the three *despite* punting 97% of its requests.
...
The `PUNT` finding suggests O_DIRECT. PostgreSQL has a developer GUC for
that. **`debug_io_direct` is not a production setting** — it exists for
PostgreSQL developers, and this experiment is included to make a point, not
as a recommendation.
...
The punts vanish completely, the `PUNT` and `WORKERS` findings disappear
from the report — and the query is no faster. Per-request latency is 2.7x
worse; identical wall time at higher per-request latency means the effective
queue depth went up.
...
uringscope tells you the mechanism changed; only the clock tells you whether
...
It is a poor throughput instrument, though — individual IOs complete far
faster than you can sample the view; 25 samples at 80ms intervals during a
cold scan mostly returned zero rows. Use `pg_aios` to catch a *stalled* AIO,
`pg_stat_io` for volume, and uringscope for rate, latency and where requests
actually went.
...
Per-op `PUNT` entries carry `evidence.op`; the summary entry does not, which
is how to tell them apart when scripting. Given everything above, gate on a
regression against a `--baseline` rather than on absolute punt rate — on
buffered PostgreSQL reads a high punt rate is the normal condition.
...
representative of bare-metal NVMe; the *shapes* — punt ratios, warm/cold
ratios, which paths use io_uring at all — are what this guide is about.
